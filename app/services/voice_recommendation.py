...
def _normalize_lang(value: str | None) -> str:
    return normalize_lang_code(value) or ""
...
def materialize_voice_replacements(
    ...
    Returns:
        tuple[dict[str, dict], dict[str, dict]]: (prepared, failures)
        - prepared: 성공한 화자들의 replacement 정보
        - failures: 실패한 화자들의 실패 원인 정보
    """
    from utils.s3 import download_from_s3
    ...
    for speaker, plan in replacements.items():
        ...
        if entry.sample_path:
            ...
            if candidate.is_file():
                ...
                failure_reason = "local_file_not_found"
                failure_details = {
                    "reason": "local_file_not_found",
                    "expected_path": str(candidate),
                    "voice_id": entry.voice_id,
                    "message": f"Voice replacement sample for {entry.voice_id} not found at {candidate}",
                }
                logger.warning(
            ...
            if not download_from_s3(bucket, entry.sample_key, local_path):
                failure_reason = "s3_download_failed"
                failure_details = {
                    ...
                    "message": f"Failed to download voice replacement sample {entry.voice_id} from s3://{bucket}/{entry.sample_key}",
                }
                logger.warning(
            ...
            failure_reason = "missing_sample_reference"
            failure_details = {
                "reason": "missing_sample_reference",
                "voice_id": entry.voice_id,
                "message": f"Voice library entry {entry.voice_id} lacks sample reference (both sample_path and sample_key are missing)",
            }
            logger.warning(
        ...
        if failure_reason:
            failures[speaker] = failure_details
            continue
...
def maybe_prepare_voice_replacements(
    ...
    prepared, failures = materialize_voice_replacements(
        paths, replacements, default_bucket
    )
    if not prepared:
